...
def _cap_antes(metodo: str, provider: Provider, banco: Banco | None) -> None:
    """A mesma checagem, na CLASSE, antes de pedir credencial.

    Sem isto, quem perguntasse ao Itaú — que não herda o mixin BACEN — levava
    `424 credenciais ausentes` e ia atrás de uma credencial que não resolveria
    nada, porque o banco não tem o produto.
    """
    exige_capacidade_antes_da_credencial(provider, banco, None, metodo,
                                         recurso="Pix Automático",
...
def criar_recorrencia(body: RecorrenciaIn, authorization: str | None = _AUTH_HEADER,
    ...
    _cap_antes("criar_recorrencia", body.provider, body.banco)
...
def listar_recorrencias(tenant_id: str = _TENANT, inicio: str = _INICIO, fim: str = _FIM,
    ...
    _cap_antes("listar_recorrencias", provider, banco)
...
def consultar_recorrencia(id_rec: str, tenant_id: str = _TENANT, txid: str | None = None,
                          provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                          credentials: str | None = _CREDS_HEADER,
                          authorization: str | None = _AUTH_HEADER,
                          vault: Vault = Depends(get_vault)) -> dict:
    """Consulta a recorrência (opcionalmente por txid da jornada)."""
    _cap_antes("consultar_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def revisar_recorrencia(id_rec: str, campos: dict, tenant_id: str = _TENANT,
                        provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                        credentials: str | None = _CREDS_HEADER,
                        authorization: str | None = _AUTH_HEADER,
                        vault: Vault = Depends(get_vault)) -> dict:
    """Gestão (Jornada 4): alteração ou cancelamento (ex.: {"status": "CANCELADA"})."""
    _cap_antes("revisar_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def criar_solicitacao(body: SolicitacaoRecorrenciaIn, authorization: str | None = _AUTH_HEADER,
    ...
    _cap_antes("criar_solicitacao_recorrencia", body.provider, body.banco)
    creds = resolve_request_credentials(authorization=authorization, explicit=body.credentials,
                                        tenant_id=body.tenant_id, provider=body.provider, banco=body.banco)
    p = _provider(body.tenant_id, body.provider, body.account_config, vault, creds,
                  banco=body.banco)
...
def consultar_solicitacao(id_solic: str, tenant_id: str = _TENANT, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                          credentials: str | None = _CREDS_HEADER,
                          authorization: str | None = _AUTH_HEADER,
                          vault: Vault = Depends(get_vault)) -> dict:
    """Consulta a solicitação de autorização."""
    _cap_antes("consultar_solicitacao_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def revisar_solicitacao(id_solic: str, campos: dict, tenant_id: str = _TENANT,
                        provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                        credentials: str | None = _CREDS_HEADER,
                        authorization: str | None = _AUTH_HEADER,
                        vault: Vault = Depends(get_vault)) -> dict:
    """Revisa/cancela a solicitação de autorização (PATCH BACEN)."""
    _cap_antes("revisar_solicitacao_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def criar_location(tenant_id: str = _TENANT, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                   credentials: str | None = _CREDS_HEADER,
                   authorization: str | None = _AUTH_HEADER,
                   vault: Vault = Depends(get_vault)) -> dict:
    """Cria a location do QR de adesão da recorrência (Jornada 2)."""
    _cap_antes("criar_location_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def consultar_location(loc_id: str, tenant_id: str = _TENANT, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                       credentials: str | None = _CREDS_HEADER,
                       authorization: str | None = _AUTH_HEADER,
                       vault: Vault = Depends(get_vault)) -> dict:
    """Consulta a location (payload do QR)."""
    _cap_antes("consultar_location_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def desvincular_location(loc_id: str, tenant_id: str = _TENANT, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                         credentials: str | None = _CREDS_HEADER,
                         authorization: str | None = _AUTH_HEADER,
                         vault: Vault = Depends(get_vault)) -> dict:
    """Desvincula a recorrência da location (invalida o QR)."""
    _cap_antes("desvincular_location_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def criar_cobranca(body: CobrancaRecorrenteIn, txid: str = _TXID,
    ...
    _cap_antes("criar_cobranca_recorrente", body.provider, body.banco)
    creds = resolve_request_credentials(authorization=authorization, explicit=body.credentials,
                                        tenant_id=body.tenant_id, provider=body.provider, banco=body.banco)
    p = _provider(body.tenant_id, body.provider, body.account_config, vault, creds,
                  banco=body.banco)
...
def listar_cobrancas(tenant_id: str = _TENANT, inicio: str = _INICIO, fim: str = _FIM,
                     provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                     credentials: str | None = _CREDS_HEADER,
                     authorization: str | None = _AUTH_HEADER,
                     vault: Vault = Depends(get_vault)) -> dict:
    ...
    inicio, fim = _periodo(inicio, fim)
    _cap_antes("listar_cobrancas_recorrentes", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def consultar_cobranca(tenant_id: str = _TENANT, txid: str = _TXID, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                       credentials: str | None = _CREDS_HEADER,
                       authorization: str | None = _AUTH_HEADER,
                       vault: Vault = Depends(get_vault)) -> dict:
    """Consulta uma cobrança do ciclo (cobr) pelo txid."""
    _cap_antes("consultar_cobranca_recorrente", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def revisar_cobranca(campos: dict, tenant_id: str = _TENANT, txid: str = _TXID, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                     credentials: str | None = _CREDS_HEADER,
                     authorization: str | None = _AUTH_HEADER,
                     vault: Vault = Depends(get_vault)) -> dict:
    """Revisa a cobrança do ciclo (PATCH BACEN) — ex.: cancelamento antes da liquidação."""
    _cap_antes("revisar_cobranca_recorrente", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def retentar_cobranca(tenant_id: str = _TENANT, txid: str = _TXID, data: date = _DATA_RETENTATIVA, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                      credentials: str | None = _CREDS_HEADER,
                      authorization: str | None = _AUTH_HEADER,
                      vault: Vault = Depends(get_vault)) -> dict:
    """Retentativa de liquidação para a data (YYYY-MM-DD) — pós vencimento não pago."""
    _cap_antes("retentar_cobranca_recorrente", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
...
def configurar_webhooks(body: dict, tenant_id: str = _TENANT, provider: Provider = _PROVIDER_ON, banco: Banco | None = _BANCO,
                        credentials: str | None = _CREDS_HEADER,
                        authorization: str | None = _AUTH_HEADER,
                        vault: Vault = Depends(get_vault)) -> dict:
    """Configura webhookrec e/ou webhookcobr: {"url_recorrencia": ..., "url_cobranca": ...}."""
    # Os dois métodos vêm do MESMO mixin: quem não tem um não tem o outro, então
    # conferir o do webhookrec já responde pelo banco inteiro. É a checagem que
    # precisa vir antes da credencial; a por método, dentro do laço, continua.
    _cap_antes("configurar_webhook_recorrencia", provider, banco)
    creds = _creds_get(credentials, authorization, tenant_id, provider, banco)
    p = _provider(tenant_id, provider, {}, vault, creds, banco=banco)
    out: dict = {}
