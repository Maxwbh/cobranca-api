...
TAGS = [
    ...
    {"name": "pix-automatico",
     "description": "Débito recorrente via Pix (BACEN): recorrência, autorização do pagador, cobranças do ciclo e retentativa. "
                    ...
                    "**Confirmado em C6 e Sicoob**, no sandbox de cada um. O **Inter** herda o mesmo dialeto, "
                    "mas ninguém verificou se o banco expõe `rec`/`solicrec` — enquanto isso ele responde `422` "
                    "dizendo *não foi confirmado* (diferente de *não oferece*), e `GET /bancos` o traz em "
                    "`capacidades_nao_confirmadas` em vez de anunciá-lo. Quem tiver credencial real confirma e "
                    "liga `INTER_PIX_AUTOMATICO_READY=true`.\n\n"
