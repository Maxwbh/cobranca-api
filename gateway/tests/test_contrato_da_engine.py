...
def test_todo_campo_que_a_api_aceita_tem_nome_publicado():
    """O inventario e o que a API ACEITA, nao o contrato que a engine publica.

    O caso acima ja comparava doc x engine e estava verde -- porque o contrato
    da engine tambem nao citava os campos. A API aceitava DOZE nomes que nem a
    engine anunciava nem a pagina publicava, e a divergencia era invisivel por
    construcao: os dois lados da comparacao vinham da mesma lista.

    Entre os doze estava `quantidade`, que no Sicoob entra no CAMPO LIVRE --
    omiti-lo num carne de tres parcelas produz codigo de barras dizendo uma, com
    DV recalculado por cima; e `logo`, que em texto abria arquivo do disco do
    servidor.

    Os apelidos ficam de fora: `conta` e `conta_corrente` sao o MESMO campo, e
    publicar os dois convida a mandar os dois -- que responde 400.
    """
    aceitos: set[str] = set()
    for slug in pycob.bancos_suportados():
        aceitos |= pycob.campos_aceitos(slug)
    apelidos = set(pycob._MAPA_CAMPOS.values())
    faltando = sorted(aceitos - _documentados() - apelidos)
    assert not faltando, (
        "a API aceita estes campos e o BoletoData da doc nao os cita: "
        f"{faltando}. Publique-os -- com o que cada um FAZ, medido: campo "
        "aceito e nao documentado e o modo como esta pagina envelhece.")
